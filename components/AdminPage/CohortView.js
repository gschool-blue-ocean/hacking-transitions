...
const CohortView = ({ studentData, currCohort, setCurrCohort, setChatCohort}) => {
  ...
  const SortButton = ({ direction, id, onClick, sortBy }) => {
    ...
    // let students = currCohort.map((currCohort)=>{
    //   return [...currCohort.students]
    // })
    ...
    const {items, requestSort, sortConfig } = useSortableData(currCohort);
            ...
            <div className={s.div}>
              <table className={s.table}>
                        <tr className={s.headtr}>
                          ...
                          <CohortFilterTable  cohort={cohort} setClickedCohort={setClickedCohort} setCurrCohort={setCurrCohort} currCohort={currCohort} setChatCohort={setChatCohort} />
                      </table>
                      </div>
          </div>
        ))}
