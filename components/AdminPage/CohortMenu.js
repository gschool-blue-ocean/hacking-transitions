...
const CohortMenu = ({ setCurrCohort, cohorts }) => {
  ...
  const handleClick = async (e) => {
    ...
    const students = (await axios.get(`/api/users/cohort/${id}`)).data;
    if (cohorts.length == 0) {
      setCurrCohort([
        {
          cohort_id: id,
    ...
    <>
      <div className={s.menucontainer}>
        <div className={s.menutitle} onClick={toggleCohortsBtn}>
          <motion.btn className={s.titlebtn}>Cohorts</motion.btn>
        </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      const cohortStudents = (
                        await axios.get(`/api/users/cohort/${cohort.cohort_id}`)
                      ).data;
